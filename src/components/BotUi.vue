<template>
  <div class="botui botui-container" v-botui-container>
    <div class="botui-messages-container">
      <div v-for="(msg,index) in messages" :key='index' class="botui-message" :class="[{human: msg.human, bot: !msg.human}, msg.cssClass]" v-botui-scroll>
      ...
      <transition name="slide-fade">
        <div v-if="action.show" v-botui-scroll>
          <form v-if="action.type == 'text'" class="botui-actions-text"
            ...
            <input ref="input" :type="action.text.sub_type" accept="image/*" capture="camera" v-model="action.text.value" class="botui-actions-text-input" :placeholder="action.text.placeholder" :size="action.text.size" :class="action.text.cssClass" required v-focus/>
            <button type="submit"
          ...
          <form v-if="action.type == 'select'" class="botui-actions-select" @submit.prevent="handle_action_select()" :class="action.cssClass">
            ...
            <select v-else v-model="action.select.value" class="botui-actions-text-select" :placeholder="action.select.placeholder" :size="action.select.size" :class="action.select.cssClass" required v-focus>
              <option v-for="(option,index) in action.select.options" :key='index' :class="action.select.optionClass" v-bind:value="option.value" :disabled="(option.value == '')?true:false" :selected="(action.select.value == option.value)?'selected':''">
                {{ option.text }}
              </option>
            </select>
          ...
          <div v-if="action.type == 'button'" class="botui-actions-buttons" :class="action.cssClass">
            <button type="button" :class="[button.icon ? 'botui-actions-buttons-button-icon' : '', button.cssClass]"
              ...
              autofocus>
              <i v-if="button.icon" class="botui-icon botui-action-button-icon fa" :class="'fa-' + button.icon" :style="'background-image:url('+button.icon+')'"></i>
               {{button.text}}
            </button>
          </div>
      ...
      directives: {
        ...
        'botui-scroll': {
          inserted: function () {
            ...
            setTimeout(() => {_container.scrollTop = _container.scrollHeight;}, 100);
          }
        },
        'focus': {
